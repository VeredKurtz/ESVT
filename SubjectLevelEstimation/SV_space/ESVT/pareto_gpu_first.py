...
df = pd.DataFrame(columns=['subject_id','ro',  'mean_M',  'mean_alpha',  'likelihood_mean'])
...
for id in all_ids:
    ...
    try:
        ...
        # Pareto
        option1_high = np.array(data_Pareto['option1_high'])
        option1_low = np.array(data_Pareto['option1_low'])
        option2_high = np.array(data_Pareto['option2_high'])
        option2_low = np.array(data_Pareto['option2_low'])
        # ro = np.array(data_Pareto['alpha'])
        actual_chosen_options_data = np.array([0 if x==1 else 1 for x in list(data_Pareto['chosen_option'])])
        ...
        num_trials = len(data_Pareto)
        
        init_M = np.median(np.concatenate((option1_high, option1_low, option2_high, option2_low)))
        # Here population Estimates
        init_alpha = 1
        initial_parameters = [init_alpha, init_M]

        # Optimization using Nelder-Mead algorithm 
        result = minimize(obj_function, initial_parameters, (option1_high, option1_low, option2_high, option2_low, actual_chosen_options_data), method='Nelder-Mead', options={"disp": True, "maxiter": 1000}, tol=0.5)

        optimized_parameters = result.x
        M_estimate = optimized_parameters[1]
        alpha_estimate = optimized_parameters[0]
            
        likelihood_mean = obj_function([alpha_estimate, M_estimate], option1_high, option1_low, option2_high, option2_low, actual_chosen_options_data)
        logging.warning("------------------------------------------------------------------------------ RESULTS ------------------------------------------------------------------------------")

        list_row = [id, ro, M_estimate,  alpha_estimate, likelihood_mean]
        df.loc[len(df)] = list_row
        df.to_csv("Estimates_Pareto.csv", index = False)
        del data
